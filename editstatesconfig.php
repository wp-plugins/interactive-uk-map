...
$states = get_option('freefreeukhtml5map_map_data');
$states = json_decode($states, true);

array_splice($states, 15);

if(isset($_POST['act_type']) && $_POST['act_type'] == 'freeuk_map_plugin_states_save') {
    ...
    foreach($states as $s_id=>$vals)
    {
        ...
        if(isset($_POST['descr'][$vals['id']]))
            update_option('freefreeukhtml5map_state_info_'.$vals['id'], stripslashes($_POST['descr'][$vals['id']]));

        if($s_id == 14) break;
    }

    update_option('freefreeukhtml5map_map_data', json_encode($states));
}

echo "<h2>" . __( 'Configuration of Map Areas', 'freefreeuk-html5-map' ) . "</h2>";
?>
<script>
...
<form method="POST" class="freeuk-html5-map">
    ...
    foreach($states as $s_id=>$vals)
    {
        ...
        elseif(stripos($vals['link'], "javascript:freeuk_map_set_state_text") === false ) $rad_other = "checked";
    ...
    ?>
    <input type="hidden" name="act_type" value="freeuk_map_plugin_states_save" />
